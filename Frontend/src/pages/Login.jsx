...
const Login = () => {
  ...
  const handleGoogle = async (e) => {
    try {
      ...
      const email = user?.email || "dummy@gmai.com";
      console.log(email);
      const verify = await axios.post(
        `${import.meta.env.VITE_BACKEND_SERVER}/student/google`,
        {
          email: email,
        },
        {
      ...
      if (verify?.data?.success) {
        localStorage.setItem("StudentData", JSON.stringify(verify?.data));
        setNotification({
          isOpen: true,
          type: "success",
          message: verify?.data?.message || "Login Successful!!",
        });
        setTimeout(() => {
          navigate("/");
        }, 2000);
